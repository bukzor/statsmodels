...
class RegressionResults(base.LikelihoodModelResults):
  ...
  **kwargs
  ...
  def rsquared_adj(self):
    ...
    """
    return 1 - (
        np.divide(self.nobs - self.k_constant, self.df_resid)
        * (1 - self.rsquared)
    )
  ...
  def bic(self):
    ...
    """
    return -2 * self.llf + np.log(self.nobs) * (self.df_model + self.k_constant)
  ...
  def eigenvals(self):
    ...
    if self._wexog_singular_values is not None:
      ...
      eigvals = np.linalg.linalg.eigvalsh(
          np.dot(self.model.wexog.T, self.model.wexog)
      )
  ...
  def summary2(
    ...
    if condno > 1000:
      warn = (
          "* The condition number is large (%.g). This might indicate \
