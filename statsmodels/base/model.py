...
class Model(object):
  ...
  def _get_init_kwds(self):
    ...
    kwds = dict(((key, getattr(self, key, None)) for key in self._init_keys))
...
class LikelihoodModel(Model):
  ...
  def _fit_zeros(
    ...
    **fit_kwds : keyword arguments
    ...
    mod_constr = self.__class__(
        self.endog, self.exog[:, keep_index], **init_kwds
    )
    ...
    res._results.normalized_cov_params[
        keep_index[:, None], keep_index
    ...
    if hasattr(res_constr, "cov_params_default"):
      ...
      res._results.cov_params_default[
          keep_index[:, None], keep_index
...
class LikelihoodModelResults(Results):
  ...
  def __init__(
      self, model, params, normalized_cov_params=None, scale=1.0, **kwargs
  ):
  ...
  def t_test(self, r_matrix, cov_p=None, scale=None, use_t=None):
    ...
    if scale is not None:
      warnings.warn(
          "scale is has no effect and is deprecated. It will"
  ...
  def wald_test(
    ...
    if use_f:
      F /= J
      return ContrastResults(
          F=F, df_denom=df_resid, df_num=J
      )  # invcov.shape[0])
    else:
      return ContrastResults(
  ...
  def wald_test_terms(
    ...
    for name, constraint in (
        constraints + combined_constraints + extra_constraints
    ):
  ...
  def t_test_pairwise(
      self, term_name, method="hs", alpha=0.05, factor_labels=None
  ):
    """
  ...
  def remove_data(self):
    ...
    value_attrs = [
        x for x in cls_attrs if isinstance(cls_attrs[x], cached_value)
    ]
...
wrap.populate_wrapper(
    LikelihoodResultsWrapper, LikelihoodModelResults  # noqa:E305
)
